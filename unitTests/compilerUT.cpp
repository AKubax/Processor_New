...
const char* const TempFName = "compilerUTTempFile.akprc";
const size_t TempFNameLen = 24;
...
int main(){
    ...
    for(dirent* curFile = readdir(dir); curFile != NULL; curFile = readdir(dir)){
        ...
        if(strcmp(dotPointer, ".akasm") != 0) continue;

        char* resFName = (char*) calloc(dotPointer - curFile->d_name + 6, 1);
        strncpy(resFName, curFile->d_name, dotPointer - curFile->d_name);
        strncpy(resFName + (dotPointer - curFile->d_name), ".akprc", 6);
